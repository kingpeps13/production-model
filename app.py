...
defaults = {
    ...
    'auto_calculate': True,
    'q_input': 1200  # добавлено для не-клея
}
...
with st.sidebar:
    ...
    if st.session_state.is_glue:
        ...
        st.session_state.q_input = st.number_input("Количество штук в заказе", 1, 100000, st.session_state.q_input, 100, key='q_input')
        Q = st.session_state.q_input
...
if st.session_state.auto_calculate:
    ...
    if st.session_state.is_glue:
        Q_calc = sum(st.session_state.gram_counts.values())
    else:
        Q_calc = st.session_state.q_input
...
if st.session_state.result:
    ...
    if r['is_glue']:
        # Показываем количество канистр и недостатки
        c1, c2, c3, c4, c5 = st.columns(5)
        c1.metric("🧴 Вес", f"{r['total_weight']:.1f} г")
        c2.metric("4-кг канистр", r['can_count_4kg'])
        c3.metric("Недостаток 4-кг", f"{r['shortage_4kg']:.1f} г" if r['shortage_4kg'] > 0 else "0 г")
        c4.metric("1-кг канистр", r['can_count_1kg'])
        c5.metric("Недостаток 1-кг", f"{r['shortage_1kg']:.1f} г" if r['shortage_1kg'] > 0 else "0 г")
    ...
    # ================== Gantt с группировкой по операциям ==================
    ...
    if r['all_intervals']:
        ...
        op_colors = {op: palette[i % len(palette)] for i, op in enumerate(r['name_list'])}
        op_colors["Наладка"] = "#7f7f7f"

        for start, end, label, _ in r['all_intervals']:
            if end <= start:
                continue
            ...
            # Исправление: для наладки берём имя операции из метки
            if label.startswith("Наладка"):
                op_name = label.replace("Наладка ", "").strip()
                display_name = label
            else:
                op_name = label.split(" (нар.")[0] if " (нар." in label else label
                display_name = label

            color = op_colors.get(op_name, "#17becf")
        ...
        finish_dt = base_dt + timedelta(hours=r['T'])
        fig.add_vline(x=finish_dt, line_width=3, line_dash="dash", line_color="red")
        fig.add_annotation(x=finish_dt, y=0.95, yref="paper",
                          text=f"Конец заказа: {r['T']:.1f} ч",
                          showarrow=False, bgcolor="white", bordercolor="red")

        fig.update_layout(
            height=max(520, len(r['name_list']) * 90),
    ...
    try:
        ...
        if r['is_glue']:
            ...
            ws1.append(["Недостаток 4-кг (г)", r['shortage_4kg']])
            ws1.append(["1-кг канистр", r['can_count_1kg']])
            ws1.append(["Недостаток 1-кг (г)", r['shortage_1kg']])
        ...
        for i, name in enumerate(r['name_list']):
            ws2.append([name, r['t_list'][i], r['setup_list'][i], r['people_list'][i],
                       r['days_work_list'][i], r['labor_details'][i][1]])
    ...
    st.info("Настройте параметры в боковой панели и включите авторасчёт")
